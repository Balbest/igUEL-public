...
c
c !--- Define body forces as a function of space coordinates
c
      Xbf = 0.d0
      Ybf = 0.d0
      Zbf = 0.d0
      do j = 1, NNODE
         Xbf = Xbf + RR(j)*COORDS(1,j)
         Ybf = Ybf + RR(j)*COORDS(2,j)
         Zbf = Zbf + RR(j)*COORDS(3,j)
      end do
      BF_x = E_mod*(1.d0 - nu_mod)/(1.d0 + nu_mod)/(1.d0 - 2.d0*nu_mod)
     *            *(1.d0 + (g_par*2.d0*Pi/4.d0)**2.d0)
     *            *0.5d0*(2.d0*Pi/4.d0)**2.d0*sin(Xbf*2.d0*Pi/4.d0)
      BF_y = E_mod*(1.d0 - nu_mod)/(1.d0 + nu_mod)/(1.d0 - 2.d0*nu_mod)
     *            *(1.d0 + (g_par*2.d0*Pi/4.d0)**2.d0)
     *            *0.5d0*(2.d0*Pi/4.d0)**2.d0*sin(Ybf*2.d0*Pi/4.d0)
      BF_z = E_mod*(1.d0 - nu_mod)/(1.d0 + nu_mod)/(1.d0 - 2.d0*nu_mod)
     *            *(1.d0 + (g_par*2.d0*Pi/4.d0)**2.d0)
     *            *0.5d0*(2.d0*Pi/4.d0)**2.d0*sin(Zbf*2.d0*Pi/4.d0)
c
