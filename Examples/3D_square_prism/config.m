% Configuration file for 3D_square_prism problem

function configs = config()

% Input geometry files (two surfaces for 3D solid)
% The surfaces must be parametrically identical
configs.geom_file1 = "InitGeom_Square4_bot.dat"; % Lower surface
configs.geom_file2 = "InitGeom_Square4_top.dat"; % Upper surface

% Output filename for Abaqus inp
configs.inp_file = "square_prism.inp";
...
configs.g = 1;       % Strain gradient parameter (used only for SGE)

% Body force vector
configs.BF_vector = [1, 1, 1]; % magnitudes in global x,y,z directions
